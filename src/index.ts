...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Bible');
}
...
const tools: McpToolExport['tools'] = [
  ...
  },
  {
    name: 'random_verse',
    description:
      'Fetch a random Bible verse from the World English Bible (WEB) translation. Returns the reference, full verse text, translation ID, and per-verse breakdown with book, chapter, and verse number.',
    inputSchema: {
      type: 'object',
      properties: {},
      required: [],
...
async function getVerse(reference: string) {
  ...
  const res = await pwFetch(`${BASE_URL}/${encodedRef}`);
  if (!res.ok) throw await httpError(res, 'Bible API error');
...
async function getPassage(reference: string, translation: string) {
  ...
  const res = await pwFetch(`${BASE_URL}/${encodedRef}?${params}`);
  if (!res.ok) throw await httpError(res, 'Bible API error');
...
async function randomVerse() {
  const res = await pwFetch(`${BASE_URL}/?random=verse`);
  if (!res.ok) throw await httpError(res, 'Bible API error');
